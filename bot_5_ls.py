"""
Bot 5 -- Build lease squares (linear regression) q-learning agent for FrozenLake.
"""
...
def main():
	...
	for episode in range(1, num_episodes + 1):
		if len(states) >= 10000:
			states, labels = [], []
		state = one_hot(env.reset(), n_obs) # initialize the game
		episode_reward = 0
		while True:
			states.append(state)
			noise = np.random.random((1, n_actions)) / episode
			action = np.argmax(Q(state) + noise)
			state2, reward, done, _ = env.step(action) # random action
			...
			state = state2 # update current state
			if len(states) % 10 == 0:
