"""
Bot 4 -- Using Q-learning network to train bot.
"""
...
def main():
	env = gym.make('FrozenLake-v0') # create game environment
	...
	with tf.Session() as session:
		...
		for episode in range(1, num_episodes + 1):
			obs_t = env.reset() # initialize the game
			episode_reward = 0
			while True:
				# 4. Take setup using best action or random action
				...
				obs_tp1, reward, done, _ = env.step(action) # random action
				...
				obs_t = obs_tp1 # update current state
